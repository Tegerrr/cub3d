...
void draw_floor_ceiling(t_data *data)
{
	...
	ceiling_rgb = set_up_color(data->map->ceiling->r, data->map->ceiling->g, data->map->ceiling->b);
	floor_rgb = set_up_color(data->map->floor->r, data->map->floor->g, data->map->floor->b);
	i = WIDTH * HEIGHT / 2 + 1;
	while (i-- > 0)
		*dest++ = ceiling_rgb;
	i = WIDTH * HEIGHT / 2 + 1;
